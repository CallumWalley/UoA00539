...
addpath(genpath(run_dir));


%% For profiling  
if exist('profile','var') && profile
    disp("profiling on");
    profile on;
    whos *;
    %path;
...
[filepath,filename,fileext] = fileparts(input_path);


%% Compile mex code
% 3 is the return type for .mex functions.
if exist('computeDists')~=3 || exist('computeDists','var')~=3 || exist('computeDists','var')~=3
...
if ~isempty(tmpDir)   
       slurmThreads = str2double(getenv('OMP_NUM_THREADS'));
       if ~isempty(slurmThreads)
...
disp([' File: ', filename]);   % File for processing
...
%                 LZ = fcnLZ(tempDataAll(jChan,:) >= median(tempDataAll(jChan,:)));
    ...
    output_path=[output_dir,'/', filename,'RS.xlsx'];
    disp(['Writing to ', output_path]);
    writetable(tableOutput, output_path,'Sheet',1,'Range','A1');
    disp('DONE!');
    profileData = profile('info');
    save([output_dir,'/profile_data.mat'], 'profileData');
